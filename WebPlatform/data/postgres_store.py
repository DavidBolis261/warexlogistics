...
class PostgresStore:
    ...
    def get_all_messages(self, limit=500):
        ...
        return pd.read_sql(
            "SELECT * FROM messages ORDER BY sent_at ASC LIMIT %(limit)s",
            self.engine,
            params={'limit': limit},
        )
        ...
        with self.engine.connect() as conn:
            if driver_id:
                result = conn.execute(text("""
                    ...
                    WHERE driver_id=:driver_id AND direction='inbound' AND is_read=FALSE
                """), {'driver_id': driver_id})
            else:
                result = conn.execute(text("""
                    SELECT COUNT(*) FROM messages WHERE direction='inbound' AND is_read=FALSE
                """))
            return result.fetchone()[0]
    ...
    def mark_messages_read(self, driver_id):
        ...
        with self.engine.connect() as conn:
            conn.execute(text("""
                ...
                WHERE driver_id=:driver_id AND direction='inbound'
            """), {'driver_id': driver_id})
            conn.commit()
    ...
    def get_driver_unread_counts(self):
        ...
        result = pd.read_sql(
            "SELECT driver_id, COUNT(*) as cnt FROM messages WHERE direction='inbound' AND is_read=FALSE GROUP BY driver_id",
            self.engine,
        )
        return dict(zip(result['driver_id'], result['cnt']))
